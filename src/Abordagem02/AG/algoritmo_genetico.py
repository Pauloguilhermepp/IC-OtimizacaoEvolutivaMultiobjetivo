...
class AlgoritmoGenetico:
    ...
    def start_pop(self):
        ...
        for i in np.arange(self.pop_size):
            self.pop[i] = Elemento(array = 
                np.random.uniform(low=self.low, high=self.high, size=self.num_genes))


    # Avaliação da população:
    def eval_pop(self):
        dados = [ [] for _ in np.arange(len(self.best_ans.fitness))]

        # Calculando desvio padrão de cada objetivo:
        for i in np.arange(self.pop_size):
            self.pop[i].fitness = self.fitness_func(self.pop[i].array)
            
            for goal in np.arange(len(self.pop[i].fitness)):
                dados[goal].append(self.pop[i].fitness[goal]) 

        self.standard_deviation = []
        
        for i in np.arange(len(dados)):
            self.standard_deviation.append(statistics.stdev(dados[i]))
        
        # Avaliando melhor indivíduo:
        for i in np.arange(self.pop_size):
            self.pop[i].fitness = self.fitness_func(self.pop[i].array)

            if(self.eh_melhor(self.pop[i].fitness, self.best_ans.fitness)):
                self.best_ans = self.pop[i]
        
        self.current_best_fit.append(self.best_ans.fitness)


    # Definindo comparação lexicográfica entre dois indivíduos:
    def eh_melhor(self, ix, parent):
        dif = []
        for i in np.arange(len(ix)):
            dif.append(ix[i] - parent[i])
        
        for i in np.arange(len(ix)):
            if(dif[i] > self.standard_deviation[i]):
                return True
    ...
    def tournament_selection(self):
        for _ in np.arange(self.pop_size):
            ...
            for ix in random.sample(range(0, self.pop_size-1), self.k_tournament - 1):
                
                if(self.eh_melhor(self.pop[ix].fitness, self.pop[parent].fitness)):
                    parent = ix
    ...
    # Mostrando gráfico:
    def show_graph(self, title = "Gráfico dos atributos do melhor indivíduo pelo número de gerações",
                xlabel = "Número de gerações", ylabel = "Atributos do melhor indivíduo"):
